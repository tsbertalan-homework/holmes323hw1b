...
def integrate(dXdt, dt, t0, tf, X0, method='euler', newtontol=1e-6):
    """
    Integrate the differential equation dXdt from t0 to tf in steps of size dt,
    ...
    ...     return np.dot(A, X)
    >>> dt=.1; t0=0; tf=4; X0=(4,8)
    >>> X, T = integrate(dXdt, dt, t0, tf, X0)
    ...
    >>> X, T = integrate(dXdt, dt, t0, tf, X0, method='ode45')
    >>> assert X.size > 2
    ...
    #  all at once. The +dt allows us to include both t0 and tf in the history.
    ...
    if method.lower() == 'rungekutta':
        ...
        #  scatter-plotting R vs t. The parts with large |dR/dt|
...
if __name__ == '__main__':
    ...
    X, T = integrate(dXdt, dt=.1, t0=0, tf=4, X0=(42, 68), method='rungekutta')
    from matplotlib import pyplot as plt
#     f = plt.figure()
#     a = f.add_subplot(1, 1, 1)
